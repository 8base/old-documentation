...
Changing/deploying to parent Environments is only possible with [Migration logic](/docs/platform-tools/ci-cd#migrations-logic-and-commands).
...
`migration generate -t <tableName>` Note that this kind of migration returns the whole state of the requested table's records without any comparison to the target Environment, so the issues may be encountered when generating migration for tables with a large number of records.
